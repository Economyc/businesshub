...
function drawRow(d, cells, color = GRAPHITE) {
...
export async function buildCountDiffPdf(data) {
    ...
    if (all.length === 0) {
        ...
        drawTableHeader(d);
        for (const l of all)
            drawRow(d, allLineCells(l), diffColor(l.diff));
    }
    ...
    if (data.lines.length === 0) {
        ...
        for (const l of data.lines)
            drawRow(d, diffLineCells(l), diffColor(l.diff));
    }
    const bytes = await pdf.save();
    return Buffer.from(bytes);
}
//# sourceMappingURL=build-count-diff-pdf.js.map