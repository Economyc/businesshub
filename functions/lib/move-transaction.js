...
import { resolveDriveUid, getUserDriveAuth, moveDriveFile, ensureFolderPath, driveClientId, driveClientSecret, DriveTokenExpiredError, DriveScopeError, } from './services/drive-oauth.js';
import { assertCompanyMember } from './utils/company-access.js';
import { monthFolderName, looseSubfolderFor, SUBFOLDER_LOOSE, SUBFOLDER_CONSOLIDATED, } from './utils/doc-naming.js';
...
function segmentsForSlot(slot, tx, year, month) {
    ...
    const docType = slot === 'paymentProof' ? 'Pago' : tx.documentKind === 'purchase' ? 'Compra' : 'Factura';
    return [year, month, SUBFOLDER_LOOSE, looseSubfolderFor(docType)];
}
export const moveInvoiceToCompany = onCall({ region: 'us-central1', memory: '512MiB', timeoutSeconds: 300, secrets: SECRETS }, async (request) => {
    if (!request.auth) {
    ...
    if (!fromCompanyId)
        throw new HttpsError('invalid-argument', 'fromCompanyId requerido');
    if (!toCompanyId)
        throw new HttpsError('invalid-argument', 'toCompanyId requerido');
    if (!transactionId)
    ...
    if (tx.splitGroupId) {
        throw new HttpsError('failed-precondition', 'Esta factura es parte de un gasto compartido entre locales. No se puede mover suelta.');
    }
    if (tx.interLocalGroupId) {
        throw new HttpsError('failed-precondition', 'Esta factura es parte de un préstamo entre locales. No se puede mover suelta.');
    }
    if (tx.sourceType === 'recurring' || tx.sourceType === 'closing') {
        throw new HttpsError('failed-precondition', 'Esta factura la generó un proceso automático (recurrente o cierre) y no se puede mover.');
    }
    ...
    const slots = ['sourceDocument', 'paymentProof', 'combinedDocument'];
    ...
    if (filesToMove.length > 0) {
        if (!toRoot) {
            throw new HttpsError('failed-precondition', 'La empresa destino no tiene Drive configurado. Conéctalo en Ajustes → Compañías antes de mover facturas con archivos.');
        }
        driveUidTo = await resolveDriveUid(toCompanyId, uid);
        const toAuth = await getUserDriveAuth(driveUidTo);
        if (!toAuth?.refreshToken) {
            throw new HttpsError('failed-precondition', 'El Drive de la empresa destino no está conectado. El propietario debe conectarlo en Ajustes → Compañías.');
        }
        ...
        if (driveUidFrom !== driveUidTo) {
            ...
            if (!fromAuth?.refreshToken) {
                throw new HttpsError('failed-precondition', 'El Drive de la empresa origen no está conectado, no se pueden recuperar los archivos para moverlos.');
            }
        }
    }
    ...
    try {
        for (const slot of filesToMove) {
            const f = tx[slot];
            const targetFolderId = await ensureFolderPath(driveUidTo, toCompanyId, toRoot, segmentsForSlot(slot, tx, year, month));
        ...
        if (err instanceof DriveTokenExpiredError) {
            throw new HttpsError('failed-precondition', 'El Drive se desconectó (sesión de Google caducada) durante el traslado. La factura NO se movió. El propietario debe reconectar Drive e intentar de nuevo.');
        }
        if (err instanceof DriveScopeError) {
            throw new HttpsError('failed-precondition', 'La conexión de Drive no tiene el permiso completo. La factura NO se movió. Reconecta marcando TODAS las casillas e intenta de nuevo.');
        }
    ...
    for (const companyId of [fromCompanyId, toCompanyId]) {
        try {
            ...
            if (companyId === fromCompanyId)
                sheetOriginRegenerated = true;
            else
                sheetTargetRegenerated = true;
            ...
            await db
                ...
                .set({ dirty: false, year: ym.year, monthIndex: ym.monthIndex, processedAt: FieldValue.serverTimestamp() }, { merge: true })
...
//# sourceMappingURL=move-transaction.js.map