...
import { resolveDriveUid, getUserDriveAuth, downloadFile, driveClientId, driveClientSecret, DriveTokenExpiredError, DriveScopeError, } from './services/drive-oauth.js';
...
export const copyInvoiceDocumentToCompany = onCall({ region: 'us-central1', memory: '512MiB', timeoutSeconds: 120, secrets: SECRETS }, async (request) => {
    if (!request.auth) {
    ...
    if (!fromAuth?.refreshToken) {
        throw new HttpsError('failed-precondition', 'El Drive de la empresa de la factura no está conectado, no se puede leer el documento para copiarlo.');
    }
    let file;
    try {
        ...
        if (err instanceof DriveTokenExpiredError) {
            throw new HttpsError('failed-precondition', 'El Drive se desconectó (sesión de Google caducada). El propietario debe reconectar Drive e intentar de nuevo.');
        }
        if (err instanceof DriveScopeError) {
            throw new HttpsError('failed-precondition', 'La conexión de Drive no tiene el permiso completo. Reconecta marcando TODAS las casillas e intenta de nuevo.');
        }
...
//# sourceMappingURL=copy-invoice-document.js.map