...
function extractContractText(data) {
    const parts = [];
    ...
    if (title)
        parts.push(`Contrato: ${title}`);
    if (employeeName)
        parts.push(`Empleado: ${employeeName}`);
    if (position)
        parts.push(`Cargo: ${position}`);
...
async function clearChunks(companyId, contractId) {
    const ref = db
...
export async function indexContract(companyId, contractId, data) {
    ...
    await parentRef.set({
        contractId,
        contractTitle: (typeof data.title === 'string' && data.title) ||
            (typeof data.templateName === 'string' && data.templateName) ||
            null,
...
export const indexContractEmbeddings = onDocumentWritten({
    ...
    try {
        ...
        console.log(`[contracts-indexer] indexed ${companyId}/${contractId}: ${result.chunks} chunks`);
    }
    catch (error) {
        console.error(`[contracts-indexer] failed for ${companyId}/${contractId}:`, error);
    }
});
//# sourceMappingURL=contracts-indexer.js.map