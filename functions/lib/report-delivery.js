...
async function writeSentReport(input, channelUsed, fellBack) {
    const ref = await db
...
export async function deliverReport(input) {
    ...
    if (input.channel === 'email') {
        if (!process.env.SENDGRID_API_KEY) {
            console.warn(`[ReportDelivery] SENDGRID_API_KEY no configurada — fallback a firestore para companyId=${input.companyId} report=${input.scheduledReportId}`);
        ...
        console.log(`[ReportDelivery] SENDGRID_API_KEY presente pero envío real no implementado — guardando en firestore. companyId=${input.companyId}`);
        const sentReportId = await writeSentReport(input, 'firestore', true);
        return {
            ok: true,
            channelUsed: 'firestore',
    ...
    if (input.channel === 'whatsapp') {
        ...
        if (!hasTwilio) {
            console.warn(`[ReportDelivery] Credenciales de Twilio no configuradas — fallback a firestore para companyId=${input.companyId} report=${input.scheduledReportId}`);
            const sentReportId = await writeSentReport(input, 'firestore', true);
            return {
                ok: true,
                channelUsed: 'firestore',
        ...
        console.log(`[ReportDelivery] Twilio presente pero envío real no implementado — guardando en firestore. companyId=${input.companyId}`);
        const sentReportId = await writeSentReport(input, 'firestore', true);
        return {
            ok: true,
            channelUsed: 'firestore',
...
//# sourceMappingURL=report-delivery.js.map