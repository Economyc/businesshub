...
import { TRANSFER_FIELDS, buildTransferRows, } from './accounting-rows.js';
...
export async function regenerateTransferSheet(companyId, year, monthIndex) {
    ...
    const driveRootFolderId = companySnap.data()
        .driveRootFolderId;
    ...
    if (!driveUid)
        return { skipped: true, reason: 'no-drive-owner' };
    ...
    if (monthTransfers.length === 0)
        return { skipped: true, reason: 'no-transfers' };
...
//# sourceMappingURL=regenerate-transfers.js.map