...
export function sheetJobRef(companyId, year, monthIndex) {
    return db
...
export function newLockOwner(tag) {
 ...
 */
export async function claimSheetJob(companyId, year, monthIndex, owner) {
 ...
 */
export async function releaseSheetJob(ref) {
    await ref
        .set({ lockedUntil: FieldValue.delete(), lockOwner: FieldValue.delete() }, { merge: true })
        .catch(() => {
...
export async function markSheetJobDirty(companyId, year, monthIndex) {
    await sheetJobRef(companyId, year, monthIndex).set({ dirty: true, year, monthIndex, updatedAt: FieldValue.serverTimestamp() }, { merge: true });
}
//# sourceMappingURL=sheet-lock.js.map