...
function resolveTargetCompany(args, defaultCompanyId, companies) {
        ...
        switch (toolName) {
            case 'createTransaction': {
                ...
                if (args.payeeType && args.payeeName) {
                    const resolution = await resolvePayeeOnCompany(companyId, args.payeeType, String(args.payeeName));
                    if (!resolution.ok) {
                ...
                const data = {
                    ...
                    status: args.status ?? (payeeRef ? 'pending' : 'paid'),
            ...
            case 'quickMarkInvoiceAsPaid': {
                ...
                if (!existing)
                    return { success: false, message: 'No encontré esa factura.' };
            ...
            case 'createPayableDocument': {
                ...
                const documentKind = args.documentKind;
                ...
                if (useCustomSupplier) {
                    ...
                    if (!resolution.ok) {
                        ...
                        return {
                            success: false,
                            message: `No encontré "${supplierName}" en proveedores. ` +
