...
const COMPANY_NAME_PARAM = z
    .string()
    .optional()
    .describe('Nombre, slug o location del local a consultar. Si se omite, usa el local activo. ' +
 ...
 */
function readToolForAnyCompany(name, build, defaultCompanyId, companies) {
    const proto = build(defaultCompanyId)[name];
    return {
        ...
        execute: async (args, options) => {
            ...
            const instance = build(companyId)[name];
            return instance.execute(rest, options);
        },
    };
}
const TARGET_COMPANY_PARAM = z
    .string()
    .optional()
    .describe('Nombre, slug o location del local donde ejecutar la operación. ' +
    'Si se omite, usa el local activo.');
export function createTelegramTools(opts) {
    const { activeCompanyId, companies, chatId } = opts;
    const read = (name, build) => readToolForAnyCompany(name, build, activeCompanyId, companies);
    const tools = {
        ...
        switchCompany: tool({
            description: 'Cambia el local activo de esta conversación de Telegram. El local activo es el default ' +
        ...
        createTransaction: tool({
            description: 'Crea una transacción financiera (ingreso o gasto). Requiere confirmación del usuario con botón. ' +
            ...
            parameters: z.object({
                ...
                payeeType: z
                    ...
                    .optional()
                    .describe('Tipo de tercero. partner=socio, employee=empleado, supplier=proveedor, external=tercero sin perfil (clientes a los que les cobramos suelen ser external).'),
                payeeName: z
                    .string()
                    .optional()
        ...
        createPayableDocument: tool({
            description: 'Crea una factura (cuenta por pagar) o una compra al contado a partir de la foto o PDF que el usuario adjuntó. ' +
        ...
        quickMarkInvoiceAsPaid: tool({
            description: 'Marca una factura pendiente como pagada SIN adjuntar comprobante. Úsala cuando el usuario dice ' +
        ...
        markInvoiceAsPaid: tool({
            description: 'Marca una factura pendiente como pagada adjuntando el comprobante que el usuario envió (foto o PDF). ' +
