...
import { invoiceButtonLabel, backToMenuKeyboard, buildDatePicker, DATE_PICKER_TEXT, isoLabel, } from './menus.js';
import { formatCop } from './format.js';
const PAGE_SIZE = 8;
function companyLabel(companies, companyId) {
...
async function resolveCompanyId(uid, chatId, companies, requested) {
    if (requested) {
    ...
    if (active && companies.some((c) => c.id === active))
        return active;
    return companies[0]?.id ?? null;
}
async function fetchPendingInvoices(companyId) {
...
function buildListKeyboard(stateId, invoices, page) {
    ...
    if (pages > 1) {
        if (safePage > 0)
            kb.text('‹', `pf:list:${stateId}:${safePage - 1}`);
...
export async function openPayFlow(ctx, deps, requestedCompanyId) {
...
async function loadPayState(ctx, stateId, chatId) {
...
export async function continuePayFlowAfterDate(ctx, deps, state, dateIso) {
    const { uid, chatId } = deps;
...
export function registerPayFlow(router) {
...
//# sourceMappingURL=pay-flow.js.map