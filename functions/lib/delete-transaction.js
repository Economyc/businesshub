...
export const deleteTransactionWithAttachments = onCall({ region: 'us-central1', memory: '512MiB', timeoutSeconds: 300, secrets: SECRETS }, async (request) => {
    if (!request.auth) {
    ...
    if (fileIds.length > 0) {
        ...
        if (!userAuth?.refreshToken) {
            throw new HttpsError('failed-precondition', 'El Drive de la empresa no está conectado. El propietario debe reconectarlo en Ajustes → Compañías antes de eliminar transacciones con adjuntos.');
        }
    }
        ...
        for (const fileId of fileIds) {
            try {
                ...
                if (err instanceof DriveTokenExpiredError) {
                    driveErrors.push('Drive se desconectó (sesión Google caducada). El propietario debe reconectarlo. La transacción se eliminó, pero los archivos quedaron en Drive.');
                    break;
                }
                if (err instanceof DriveScopeError) {
                    driveErrors.push('La conexión de Drive no tiene el permiso completo. Reconecta marcando TODAS las casillas. La transacción se eliminó, pero los archivos quedaron en Drive.');
                    break;
                }
