...
async function fetchSalesInRange(companyId, startDate, endDate, localId) {
    const snap = await db
    ...
    for (const doc of snap.docs) {
        const data = doc.data();
        if (localId !== undefined && String(data.localId) !== String(localId))
            continue;
...
function dateFromFecha(fecha) {
    if (!fecha)
        return null;
    // Formato POS típico: "YYYY-MM-DD HH:mm:ss" o "YYYY-MM-DD".
    return fecha.slice(0, 10);
}
export function createPosTools(companyId) {
    return {
        getPosSales: tool({
            description: 'Obtiene las ventas del POS en un rango de fechas con agregación por día: total vendido, cantidad de ventas y ticket promedio. Puede filtrar por local.',
            parameters: z.object({
        ...
        getSalesByPaymentMethod: tool({
            description: 'Desglosa las ventas del POS por método de pago (AP, QR, datáfono, efectivo, Rappi, etc.) en un rango de fechas. Suma los montos de `pagosList` de cada venta.',
            parameters: z.object({
                startDate: z.string().describe('Fecha inicio (YYYY-MM-DD)'),
                endDate: z.string().describe('Fecha fin (YYYY-MM-DD)'),
            }),
        ...
        getTopProducts: tool({
            description: 'Lista los productos más vendidos del POS en un rango de fechas, agregados por unidades vendidas y valor total.',
            parameters: z.object({
                startDate: z.string().describe('Fecha inicio (YYYY-MM-DD)'),
                endDate: z.string().describe('Fecha fin (YYYY-MM-DD)'),
        ...
        getPosSyncStatus: tool({
            description: 'Estado de sincronización del POS: última fecha con datos, huecos recientes, total de meses cacheados.',
            parameters: z.object({}),
            execute: async () => {
        ...
        triggerPosReconcile: tool({
            description: 'Dispara una reconciliación del POS (descarga ventas recientes al caché). Requiere confirmación del usuario. Por defecto reconcilia los últimos 7 días.',
            parameters: z.object({
...
//# sourceMappingURL=pos-tools.js.map