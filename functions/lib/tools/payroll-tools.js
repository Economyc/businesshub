...
export function createPayrollTools(companyId) {
    return {
        generatePayrollPreview: tool({
            description: 'Genera un preview de nómina para un mes específico. Calcula salario, auxilio de transporte, deducciones de salud y pensión para cada empleado activo. NO escribe datos — solo calcula y retorna el preview.',
            parameters: z.object({
            ...
            execute: async ({ year, month }) => {
                ...
                const existing = payrolls.find((p) => Number(p.year) === year && Number(p.month) === month);
                if (existing) {
                    return {
                        error: true,
        ...
        createPayrollDraft: tool({
            description: 'Crea un borrador de nómina en el sistema. Requiere confirmación del usuario. Usa los datos previamente calculados por generatePayrollPreview.',
            parameters: z.object({
...
//# sourceMappingURL=payroll-tools.js.map