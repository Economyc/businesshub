...
export function createNotificationsTools(companyId) {
    return {
        getNotifications: tool({
            description: 'Obtiene las notificaciones internas del sistema (reportes semanales, alertas de mora, recordatorios de cierre, aumentos de precio). Puede filtrar por tipo o por no leídas.',
            parameters: z.object({
        ...
        createNotification: tool({
            description: 'Crea una nueva notificación interna (útil para dejar recordatorios al usuario). Requiere confirmación del usuario.',
            parameters: z.object({
                type: z
                    .enum(['weekly-report', 'overdue-alert', 'closing-reminder', 'price-increase'])
