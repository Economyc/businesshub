...
export function createThreadTools(companyId, threadId) {
    return {
        updateThreadState: tool({
            ...
            parameters: z.object({
                contextPatch: z
                    .record(z.unknown())
                    .optional()
                    .describe('Merge sobre el campo context del thread. Usa keys descriptivas (ej: { "mesEnCurso": "abril 2026", "pendientes": ["nomina","prestaciones"] }). No envíes el objeto completo, solo los cambios.'),
            ...
            execute: async ({ contextPatch, nextActionsAddOrRemove, summary, status }) => {
                ...
                let nextActionsAfter = null;
                if (nextActionsAddOrRemove && (nextActionsAddOrRemove.add || nextActionsAddOrRemove.remove)) {
                    const snap = await ref.get();
                    const current = (snap.exists && Array.isArray(snap.data().nextActions)
                        ? snap.data().nextActions.filter((a) => typeof a === 'string')
                        : []);
