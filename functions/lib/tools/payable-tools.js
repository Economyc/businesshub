...
export function createPayableTools(companyId) {
    return {
        createPayableDocument: tool({
            description: 'Crea una factura (cuenta por pagar) o una compra al contado a partir de un archivo adjunto (imagen o PDF). Requiere confirmación del usuario. ' +
        ...
        findMatchingPayables: tool({
            description: 'Busca cuentas por pagar pendientes (Facturas en estado pending) que matcheen con un proveedor y un monto dados. ' +
            ...
            execute: async ({ supplierName, amount, amountTolerance = 0 }) => {
                ...
                const matches = txSnap.docs
                    ...
                    supplierName: t.payeeRef?.name ?? null,
                    sourceDocumentLink: t.sourceDocument?.driveWebViewLink ?? null,
                }));
                return {
        ...
        markInvoiceAsPaid: tool({
            description: 'Marca una factura pendiente como pagada y adjunta el comprobante de pago. Requiere confirmación del usuario. ' +
...
//# sourceMappingURL=payable-tools.js.map