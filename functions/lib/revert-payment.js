...
export const revertPaymentWithAttachments = onCall({ region: 'us-central1', memory: '512MiB', timeoutSeconds: 300, secrets: SECRETS }, async (request) => {
    if (!request.auth) {
    ...
    const fileIds = Array.from(new Set([reverted.proof?.driveFileId, tx.combinedDocument?.driveFileId].filter((id) => typeof id === 'string' && id.length > 0)));
    ...
    if (fileIds.length > 0) {
        ...
        if (!userAuth?.refreshToken) {
            throw new HttpsError('failed-precondition', 'El Drive de la empresa no está conectado. El propietario debe reconectarlo en Ajustes → Compañías antes de revertir abonos con adjuntos.');
        }
    }
        ...
        for (const fileId of fileIds) {
            try {
                ...
                if (err instanceof DriveTokenExpiredError) {
                    driveErrors.push('Drive se desconectó (sesión Google caducada). El propietario debe reconectarlo. El abono se revirtió, pero los archivos quedaron en Drive.');
                    break;
                }
                if (err instanceof DriveScopeError) {
                    driveErrors.push('La conexión de Drive no tiene el permiso completo. Reconecta marcando TODAS las casillas. El abono se revirtió, pero los archivos quedaron en Drive.');
                    break;
                }
    ...
    const monthCandidates = [monthOf(reverted.date), monthOf(tx.paidDate), currentYm()].filter((m) => m != null);
    const monthsByKey = new Map();
    for (const m of monthCandidates)
        monthsByKey.set(ymKey(m.year, m.monthIndex), m);
    const monthsRegenerated = [];
...
//# sourceMappingURL=revert-payment.js.map