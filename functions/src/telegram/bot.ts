...
import { createCallbackRouter, type CallbackDeps } from './callbacks.js'
 ...
 */
async function loadCategoriesBlock(): Promise<string> {
  try {
    const snap = await db.collection('settings').doc('categories').get()
    const items = (snap.data()?.categories ?? []) as Array<{
      name?: string
...
let suppliersBlockCache: { value: string; at: number } | null = null

async function loadSuppliersBlock(companyId: string): Promise<string> {
  const now = Date.now()
  ...
  try {
    ...
    if (active.length > 0 && active.length <= SUPPLIERS_MAX) {
      const lines = active.map((s) =>
        s.category ? `- ${s.name} → categoría: ${s.category}` : `- ${s.name}`,
      )
      value = [
      ...
      content: [
        {
          ...
          result: {
            success: false,
            message:
              'El usuario respondió en vez de confirmar; esta propuesta quedó descartada. ' +
...
export function createTelegramBot(cfg: BotConfig): Bot<BotContext> {
  ...
  router.register('m', async (ctx, args, deps) => {
    ...
    if (action === 'home') {
      ...
      if (selector) await ctx.editMessageText(selector.text, { reply_markup: selector.keyboard }).catch(() => {})
      else await ctx.editMessageText('Tu usuario no tiene empresas activas.').catch(() => {})
    } else if (action === 'pdf') {
  ...
  bot.command('empresa', async (ctx) => {
    ...
    if (!arg) {
      ...
      if (selector) await ctx.reply(selector.text, { reply_markup: selector.keyboard })
      else await ctx.reply('Tu usuario no tiene empresas activas.')
      return
    }
  ...
  bot.on('callback_query:data', async (ctx) => {
    ...
    const chatId = ctx.chat!.id
    const uid = ctx.state.uid
    const deps: CallbackDeps = { cfg, chatId, uid }
  ...
  bot.on('message', async (ctx) => {
    ...
    try {
      ...
      if (state.awaitingQuickEntry && text && !fileId) {
        ...
        const userMessage = { role: 'user', content: [{ type: 'text', text }] } as unknown as CoreMessage
        const tools = createTelegramTools({ activeCompanyId, companies, chatId })
        const turn = await runAgentTurn({
