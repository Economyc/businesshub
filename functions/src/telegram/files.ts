...
export async function downloadTelegramFile(
  ...
  if (!info.ok || !info.result?.file_path) {
    throw new TelegramFileError(
      `No pude obtener el archivo de Telegram (${info.description ?? 'getFile falló'}). ` +
  ...
  if ((info.result.file_size ?? 0) > maxBytes) {
    throw new TelegramFileError(
      `El archivo pesa más de ${Math.round(maxBytes / 1024 / 1024)}MB — envía una versión más liviana.`,
    )
  }
