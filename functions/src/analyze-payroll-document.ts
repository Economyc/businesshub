...
interface AnalyzeInput {
  companyId: string
  kind: PayrollKind
  /** PDF/imagen en base64. Requerido salvo que se envíe spreadsheetText. */
  fileBase64?: string
  mimeType?: string
  /** Texto ya parseado de un Excel/CSV de propinas (alternativa a fileBase64). */
...
const ColillaSchema = z.object({
  ...
  totalDevengado: copNumber.describe(
    'TOTAL DEVENGADO en pesos colombianos enteros. Ej: "1.197.773" → 1197773. 0 si no es claro.',
  ),
  totalDeducciones: copNumber.describe(
    'TOTAL DEDUCCIONES en pesos colombianos enteros. Ej: "70.036" → 70036. 0 si no aparece.',
  ),
  netoCancelado: copNumber.describe(
    'NETO CANCELADO / neto a pagar en pesos colombianos enteros. Ej: "1.127.736" → 1127736. 0 si no es claro.',
  ),
})
  ...
  rows: z
    .array(
      z.object({
        ...
        amount: copNumber.describe(
          'Valor de propina en pesos colombianos enteros. Ej: "594.221" → 594221.',
        ),
      }),
    )
...
type ColillaExtraction = z.infer<typeof ColillaSchema>
type PropinasExtraction = z.infer<typeof PropinasSchema>

const EMPTY_COLILLA: ColillaExtraction = {
  employeeName: '',
  identification: '',
  role: '',
  payPeriod: '',
...
const EMPTY_PROPINAS: PropinasExtraction = { rows: [], total: 0 }

const COLILLA_PROMPT =
  'Este documento es un desprendible/colilla de liquidación de nómina de UN empleado. ' +
  'Extrae los campos:\n' +
...
async function extractFromText<T>(
  ...
  throw new ExtractionFailedError([
    { provider: 'text-only', error: (lastErr as Error)?.message ?? 'sin proveedores disponibles' },
  ])
}
