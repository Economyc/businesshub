...
export function driveReqOpts(opts: DriveOpts | undefined, capMs?: number) {
...
export function isRetryableDriveError(err: unknown): boolean {
  ...
  if (isInvalidGrant(err) || isInsufficientScope(err)) return false
  ...
  if (
    reasons.some((r) =>
      ['rateLimitExceeded', 'userRateLimitExceeded', 'backendError', 'internalError'].includes(
        r ?? '',
      ),
    )
  ) {
 ...
 */
export async function withDriveRetry<T>(fn: () => Promise<T>, opts?: DriveOpts): Promise<T> {
  ...
  for (let attempt = 1; ; attempt++) {
    if (Date.now() + attemptTimeoutMs > deadlineAt) throw new DriveBudgetExceededError()
    try {
      return await fn()
    } catch (err) {
      ...
      if (Date.now() + delay + attemptTimeoutMs > deadlineAt) throw new DriveBudgetExceededError()
      console.warn(
        `[drive-retry] intento ${attempt}/${maxAttempts} falló (${(err as Error)?.message}); reintento en ${Math.round(delay)}ms`,
      )
...
export async function ensureFolderPath(
  ...
  for (const seg of segments) {
    ...
    const folderId = await runDrive(uid, () =>
      withDriveRetry(() => findOrCreateFolder(drive, parent, seg, opts), opts),
    )
