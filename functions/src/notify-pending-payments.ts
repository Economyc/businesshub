...
function bogotaLabel(date: Date): string {
...
async function buildCompanySection(companyId: string, companyName: string): Promise<PendingCompany | null> {
  const snap = await db
...
function buildCaption(report: PendingReport): string {
  const lines = [
