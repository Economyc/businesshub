...
function extractCompanyId(event: unknown): string | null {
...
async function findRoleDefinition(
  ...
  for (const c of companies.docs) {
    if (c.id === excludeCompanyId) continue
...
export const seedGlobalMembersOnCompanyCreate = onDocumentCreated(
  ...
  async (event) => {
    const companyId =
      (event.params as { companyId?: string } | undefined)?.companyId ?? extractCompanyId(event)
    ...
    for (const g of globals.docs) {
      const data = g.data() as GlobalMember
      ...
      if (!(await roleRef.get()).exists) {
        ...
        if (definition) {
          ...
          console.warn(
            `[seedGlobalMembers] ${companyId}: no se encontró definición del rol '${roleId}' en ninguna compañía`,
          )
        }
      }
      ...
      console.log(
        `[seedGlobalMembers] ${companyId} → ${data.email ?? uid} como '${roleId}'`,
      )
    }
  },
)
