...
// Sube la hoja de seguimiento de facturas/pagos a Drive como Google Sheet
// nativo, en la carpeta del mes {root}/{YYYY}/{MesEs}/ (junto a los PDFs).
// Un archivo por mes: si ya existe uno con el mismo nombre, se reemplaza (no se
// duplica). La conversión .xlsx → Google Sheet usa solo el scope de Drive, así
// que NO requiere el scope de Sheets ni que nadie reconecte.
...
export const saveInvoiceSheetToDrive = onCall(
  ...
  async (request) => {
    ...
    if (!data.fileName?.trim()) throw new HttpsError('invalid-argument', 'fileName requerido')
    if (
    ...
    const company = companySnap.data() as { driveRootFolderId?: string }
