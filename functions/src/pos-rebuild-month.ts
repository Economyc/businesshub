...
export const posRebuildMonth = onCall<RebuildData>(
  {
    timeoutSeconds: 3600,
    memory: '1GiB',
  ...
  async (req) => {
    ...
    if (lastRun) {
      ...
      if (ageMs < MANUAL_COOLDOWN_MS) {
        ...
        throw new HttpsError(
          'resource-exhausted',
          `Hay un reconcile reciente. Espera ${waitS}s antes de reconstruir.`,
        )
      }
    }
    ...
    try {
      ...
      const emptyPrev: PreviousCounts = new Map()
