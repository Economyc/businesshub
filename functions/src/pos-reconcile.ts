...
async function reconcileCompany(opts: ReconcileCompanyOptions): Promise<ReconcileStats> {
  ...
  await metaRef
    ...
    .catch((err) => {
      ...
      console.error(
        `[PosReconcile] no se pudo marcar inProgress company=${opts.companyId}: ${err}`,
      )
    })
  ...
  try {
      ...
      for (const window of opts.windows) {
        ...
        while (r.rateLimited && attempt < MAX_WINDOW_RETRIES) {
          ...
          const backoff = Math.min(
            WINDOW_BACKOFF_BASE_MS * 2 ** (attempt - 1),
            WINDOW_BACKOFF_CAP_MS,
          )
          console.warn(
        ...
        if (r.rateLimited) {
          ...
          console.warn(
            `[PosReconcile] rate-limit persistente tenant=${opts.tenantId} company=${opts.companyId} ` +
    ...
    await metaRef
      ...
      .catch((err) => {
        ...
        console.error(
          `[PosReconcile] no se pudo limpiar inProgress company=${opts.companyId}: ${err}`,
        )
      })
  }
...
export const posReconcileOnDemand = onCall<OnDemandData>(
  ...
  async (req) => {
    ...
    const lastRun = meta?.lastRun as Timestamp | undefined
    if (lastRun) {
    ...
    const startedAt = meta?.startedAt as Timestamp | undefined
    if (
