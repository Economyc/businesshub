...
function monthForTransfer(tr: TransferLike): { year: number; monthIndex: number } | null {
  try {
...
export const deleteTransferWithAttachments = onCall(
  ...
  async (request): Promise<DeleteResult> => {
    ...
    if (!transferId) throw new HttpsError('invalid-argument', 'transferId requerido')
    ...
    const tr = snap.data() as TransferLike

    // Único adjunto posible: el comprobante del traslado.
    const fileIds = Array.from(
      new Set(
        [tr.proof?.driveFileId].filter(
    ...
    if (fileIds.length > 0) {
      ...
      if (!userAuth?.refreshToken) {
        throw new HttpsError(
          'failed-precondition',
          'El Drive de la empresa no está conectado. El propietario debe reconectarlo en Ajustes → Compañías antes de eliminar traslados con comprobante.',
        )
      }
    }
      ...
      for (const fileId of fileIds) {
        try {
          ...
          if (err instanceof DriveTokenExpiredError) {
            driveErrors.push(
              'Drive se desconectó (sesión Google caducada). El propietario debe reconectarlo. El traslado se eliminó, pero el archivo quedó en Drive.',
            )
            break
          }
          if (err instanceof DriveScopeError) {
            driveErrors.push(
              'La conexión de Drive no tiene el permiso completo. Reconecta marcando TODAS las casillas. El traslado se eliminó, pero el archivo quedó en Drive.',
            )
            break
          }
