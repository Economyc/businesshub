...
interface AnomalyPayload {
  kind: 'expense-outlier' | 'sales-drop' | 'contract-expiring'
  severity: 'info' | 'warning'
  title: string
  description: string
  evidence: Record<string, unknown>
  // Opaque hash for deterministic ID — distinto por evento concreto (categoría,
  // contrato, etc.) para que dos anomalías del mismo tipo el mismo día no se
  // pisen entre sí.
  hashSeed: string
}
...
// ─── Anomalía 1: gasto > media + 2σ por categoría ───
...
async function detectExpenseOutliers(
  companyId: string,
  runDate: string,
  now: Date,
): Promise<number> {
  ...
  const stats = new Map<string, { mean: number; stddev: number; n: number }>()
  for (const [cat, amounts] of byCat) {
    if (amounts.length < 5) continue // sin suficientes muestras, no juzgamos
...
async function detectSalesDrop(
  companyId: string,
  runDate: string,
  now: Date,
): Promise<number> {
  ...
  for (const doc of snap.docs) {
    const data = doc.data() as { date?: string; ventas?: Array<{ total?: unknown }> }
...
async function detectExpiringContracts(
  ...
  for (const c of contracts) {
    ...
    if (status === 'terminated' || status === 'cancelled' || status === 'cancelado') continue
