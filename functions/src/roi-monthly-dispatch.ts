...
export async function generateRoiForCompany(companyId: string, now: Date): Promise<number> {
  const snap = await db
  ...
  if (snap.empty) return 0 // la company no maneja ROI
  ...
  const byMonth = new Map<number, RoiTx[]>()
  for (const doc of snap.docs) {
    const tx = doc.data() as RoiTx
  ...
  const sourceKey = Math.max(
    ...[...byMonth.keys()].filter((k) => k < targetKey).concat(-Infinity),
  )
  if (sourceKey === -Infinity) return 0

  // Un ROI por socio: si el mes fuente tiene varios del mismo socio, se suman.
  const bySocio = new Map<string, { payeeRef: PayeeRef; amount: number }>()
  for (const tx of byMonth.get(sourceKey) ?? []) {
    const id = tx.payeeRef?.id
    if (!id) continue
    const entry = bySocio.get(id) ?? { payeeRef: tx.payeeRef as PayeeRef, amount: 0 }
  ...
  const yaGenerados = new Set(
    (byMonth.get(targetKey) ?? []).map((tx) => tx.payeeRef?.id).filter(Boolean),
  )
  ...
  for (const [id, { payeeRef, amount }] of bySocio) {
    if (yaGenerados.has(id) || amount <= 0) continue
  ...
  if (generated > 0) {
    console.log(
      `[roi-monthly] ${companyId}: ${generated} generados para ${monthLabel(targetKey)} (fuente ${monthLabel(sourceKey)})`,
    )
  }
  return generated
}
