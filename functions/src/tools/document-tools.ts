...
export function createDocumentTools(companyId: string) {
  return {
    getContracts: tool({
      ...
      execute: async ({ employeeName, status }) => {
        ...
        if (employeeName) {
          ...
          filtered = filtered.filter((c) =>
            String(c.employeeName ?? '').toLowerCase().includes(search)
          )
        }
        if (status) {
    ...
    createContractFromTemplate: tool({
      description:
        'Genera un contrato para un empleado desde una plantilla. Requiere confirmación del usuario. El PDF se genera luego desde la UI.',
      parameters: z.object({
