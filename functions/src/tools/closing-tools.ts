...
function groupByCategory(txs: Record<string, unknown>[]) {
...
export function createClosingTools(companyId: string) {
  return {
    generateMonthClosingPreview: tool({
      description:
        'Genera un preview del cierre de mes: resumen financiero (P&L), transacciones recurrentes pendientes, items vencidos, y estado de nómina. NO ejecuta ninguna acción.',
      parameters: z.object({
    ...
    executeMonthClosing: tool({
      description:
        'Ejecuta el cierre de mes: genera transacciones recurrentes pendientes. Requiere confirmación del usuario. Usa los datos del preview generado por generateMonthClosingPreview.',
      parameters: z.object({
        year: z.number().describe('Año del cierre'),
        month: z.number().min(1).max(12).describe('Mes del cierre (1-12)'),
