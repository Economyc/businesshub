...
function formatClosing(c: Record<string, unknown>) {
...
export function createDailyClosingTools(companyId: string) {
  return {
    getDailyClosings: tool({
      ...
      execute: async ({ startDate, endDate }) => {
        ...
        const filtered = all
          .filter((c) => inRange(c.date as string | undefined, startDate, endDate))
          .map(formatClosing)
    ...
    getDiscountsReport: tool({
      description:
        'Reporte de descuentos aplicados en un rango. Descuentos pueden ser de tipo parcial o total, con razones: Empleado, Influencer, Socio, Prueba de calidad, Otro.',
      parameters: z.object({
        startDate: z.string().describe('Fecha inicio (YYYY-MM-DD)'),
        endDate: z.string().describe('Fecha fin (YYYY-MM-DD)'),
        reason: z
      ...
      execute: async ({ startDate, endDate, reason }) => {
        ...
        let filtered = all.filter((d) => inRange(d.date as string | undefined, startDate, endDate))
        if (reason) filtered = filtered.filter((d) => d.reason === reason)
        const totalAmount = filtered.reduce((s, d) => s + (Number(d.amount) || 0), 0)
        const byReason = filtered.reduce<Record<string, { count: number; amount: number }>>((acc, d) => {
    ...
    getTipsSummary: tool({
      ...
      execute: async ({ startDate, endDate }) => {
        const all = await fetchCollection(companyId, 'closings')
        const filtered = all.filter((c) => inRange(c.date as string | undefined, startDate, endDate))
        ...
        const byDay = filtered
          .map((c) => ({ date: c.date as string, tips: Number(c.propinas) || 0 }))
    ...
    createDailyClosing: tool({
      description:
        'Crea un cierre diario con desglose por método de pago. Requiere confirmación del usuario. Sincroniza transacciones automáticamente.',
      parameters: z.object({
        date: z.string().describe('Fecha del cierre (YYYY-MM-DD)'),
