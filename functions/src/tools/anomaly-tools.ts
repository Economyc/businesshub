...
function tsToMillis(val: unknown): number {
  if (val && typeof val === 'object' && '_seconds' in val) {
...
export function createAnomalyTools(companyId: string) {
  return {
    getDetectedAnomalies: tool({
      description:
        'Lista las anomalías detectadas automáticamente por el sistema (gastos inusuales, caídas de ventas, contratos por vencer). Úsala cuando el usuario pregunte "¿hay algo raro?", "¿qué alertas hay?", "¿algo fuera de lo normal?".',
      parameters: z.object({
    ...
    acknowledgeAnomaly: tool({
      description:
        'Marca una anomalía como atendida (acknowledged=true). Úsala cuando el usuario diga "ya la vi", "atendida", "ignórala". Requiere confirmación del usuario.',
      parameters: z.object({
