...
export function createAnalysisTools(companyId: string) {
  return {
    ...
    analyzeSupplierPrices: tool({
      description:
        'Analiza el historial de compras por proveedor o categoría. Muestra tendencia de precios y volumen de compras.',
      parameters: z.object({
        months: z
          .number()
          .optional()
      ...
      execute: async ({ months }) => {
        ...
        // Group by supplier
        const supplierMap = new Map<string, { name: string; purchases: Record<string, unknown>[] }>()
        ...
        const supplierAnalysis = Array.from(supplierMap.entries()).map(
          ([id, { name, purchases: pList }]) => {
