...
export function createBankTools(companyId: string) {
  return {
    getBankReconcileStatus: tool({
      description:
        'Lista los extractos bancarios importados de este local con su periodo, banco, conteo de movimientos y estado de conciliación (imported = sin conciliar, reconciled = ya conciliado). Útil para responder "qué extractos tengo", "cuál falta conciliar" y para obtener el statementId antes de conciliar.',
      parameters: z.object({}),
      execute: async () => {
        const snap = await db
    ...
    getBankMovements: tool({
      description:
        'Obtiene los movimientos de un extracto bancario importado, con agregados por dirección (entradas/salidas) y por estado de conciliación. Si no se pasa statementId usa el extracto más reciente. Útil para "qué entró en el extracto de mayo", "qué quedó sin conciliar".',
      parameters: z.object({
        statementId: z
          .string()
          .optional()
      ...
      execute: async ({ statementId, onlyUnreconciled, limit = 50 }) => {
        ...
        if (!stmtId) {
          ...
          if (stmts.empty) return { found: false, message: 'No hay extractos importados.' }
          stmtId = stmts.docs
            .sort((a, b) =>
              (tsToISO(b.data().periodEnd) ?? '').localeCompare(tsToISO(a.data().periodEnd) ?? ''),
            )[0].id
        }
        ...
        if (onlyUnreconciled) {
          movements = movements.filter(
            (m) => m.reconcileStatus === 'pending' || m.reconcileStatus === 'partial',
          )
        }
        ...
        return {
          ...
          byStatus: movements.reduce<Record<string, number>>((acc, m) => {
