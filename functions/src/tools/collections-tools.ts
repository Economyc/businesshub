...
export function createCollectionsTools(companyId: string) {
  return {
    getOverdueCollections: tool({
      description:
        'Obtiene la lista priorizada de cuentas por cobrar vencidas o pendientes. Incluye días de mora, montos y plantillas de mensaje de cobro sugeridas para WhatsApp/email.',
      parameters: z.object({
        minDaysOverdue: z
          .number()
          .optional()
