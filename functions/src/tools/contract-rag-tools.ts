...
interface ScoredChunk extends ChunkDoc {
  score: number
...
async function loadChunks(
  companyId: string,
  contractId?: string,
): Promise<{ chunks: ChunkDoc[]; titles: Map<string, string | null> }> {
  ...
  if (contractId) {
    ...
    titles.set(contractId, (parentSnap.data()?.contractTitle as string | null) ?? null)
  ...
  for (const p of parents.docs) {
    titles.set(p.id, (p.data()?.contractTitle as string | null) ?? null)
  }
  // Lee todos los chunks con un collectionGroup-style traversal por padre.
  const subSnaps = await Promise.all(
    parents.docs.map((p) => p.ref.collection('chunks').get()),
  )
...
export function createContractRagTools(companyId: string) {
  return {
    searchContracts: tool({
      description:
        'Busca pasajes relevantes en los contratos indexados (RAG semántico). Devuelve top K chunks con score de similitud. Úsalo para responder preguntas sobre cláusulas, condiciones, salarios o detalles específicos. Cita siempre el contractId y chunkIndex en la respuesta.',
      parameters: z.object({
    ...
    summarizeContract: tool({
      description:
        'Resume un contrato indexado en 5 viñetas. Útil cuando el usuario pide un resumen completo en vez de buscar una cláusula puntual.',
      parameters: z.object({
