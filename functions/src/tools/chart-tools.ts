...
export function createChartTools() {
  return {
    generateChart: tool({
      description:
        'Genera un gráfico visual dentro del chat. Usa esta herramienta DESPUÉS de obtener datos con otra herramienta. Envía los datos ya procesados para que el frontend los renderice como gráfico.',
      parameters: z.object({
        chartType: z.enum(['bar', 'pie', 'area', 'line']).describe(
          'Tipo de gráfico: bar (comparaciones), pie (distribución %), area (tendencias con relleno), line (tendencias simples)'
        ),
