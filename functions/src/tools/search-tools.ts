...
export function createSearchTools(companyId: string) {
  return {
    searchAll: tool({
      description:
        'Busca un término en todas las colecciones: empleados, proveedores, transacciones y contratos. Útil para encontrar información rápidamente sin saber en qué módulo está.',
      parameters: z.object({
      ...
      execute: async ({ query }) => {
        ...
        const matchedTransactions = transactions
          .filter((t) =>
            matchesSearch(t.concept, search) ||
