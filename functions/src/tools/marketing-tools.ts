...
function inRange(val: unknown, start: string, end: string): boolean {
  const d = tsToDate(val)
  if (!d) return false
...
export function createMarketingTools(companyId: string) {
  return {
    getInfluencerVisits: tool({
      description:
        'Lista las visitas de influencers registradas. Incluye nombre, redes sociales, fecha, orden vinculada, checklist de contenido (story/post/reel) y estado (pending/completed).',
      parameters: z.object({
    ...
    getInfluencerContentReport: tool({
      description:
        'Reporte de contenido generado por influencers en un rango: cuenta de stories, posts y reels; top influencers por contenido.',
      parameters: z.object({
      ...
      execute: async ({ startDate, endDate }) => {
        ...
        let posts = 0
        let reels = 0
        const byInfluencer = new Map<string, { visits: number; content: number }>()
        for (const v of filtered) {
          const content = (v.content as { story?: boolean; post?: boolean; reel?: boolean } | undefined) ?? {}
          const contentCount = (content.story ? 1 : 0) + (content.post ? 1 : 0) + (content.reel ? 1 : 0)
          if (content.story) stories += 1
          if (content.post) posts += 1
          if (content.reel) reels += 1
