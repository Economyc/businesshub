 ...
 */
export async function generatePendingTransactionsServer(companyId: string): Promise<number> {
  ...
  const allRecurring = recurringSnap.docs.map((d) => ({ id: d.id, ...d.data() } as RawRecurring))
  ...
  for (const recurring of active) {
    ...
    let lastGenerated: Date | null = tsToDate(recurring.lastGeneratedDate) ?? null
    ...
    while (startOfDay(nextDue) <= today) {
      ...
      nextDue = addFrequency(nextDue, recurring.frequency as RecurrenceFrequency)
      hasChanges = true
      generated++
    }
 ...
 */
export async function countPendingRecurring(companyId: string): Promise<number> {
  const recurringSnap = await db
    .collection('companies')
    .doc(companyId)
    .collection('recurring-transactions')
