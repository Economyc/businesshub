...
function drawTableHeader(d: Doc, cols: Col[]): void {
  ensureSpace(d, ROW_H)
...
function drawRow(d: Doc, cols: Col[], cells: string[], color = GRAPHITE): void {
...
function sectionTitle(d: Doc, title: string, size = 11): void {
...
function subTitle(d: Doc, title: string): void {
  ensureSpace(d, ROW_H)
  d.page.drawText(title, { x: MARGIN, y: d.y - 9, size: 9, font: d.bold, color: MID })
  d.y -= ROW_H
}

function emptyNote(d: Doc, text: string): void {
  ensureSpace(d, ROW_H)
...
function drawCompany(d: Doc, c: PendingCompany): void {
  ...
  if (c.invoiceSuppliers.length === 0) {
    ...
    for (const s of c.invoiceSuppliers) {
      ...
      drawRow(
        d,
        INVOICE_COLS,
        [s.supplierName, String(s.count), fmtMoney(s.total), s.oldestDate ?? '—', overdue],
        s.overdueCount > 0 ? NEGATIVE : GRAPHITE,
      )
    }
  }
  ...
  if (c.obligations.length === 0) {
    ...
    for (const o of c.obligations) {
      drawRow(
        d,
        OBLIGATION_COLS,
        [o.concept || '—', o.dueDate ?? '—', fmtMoney(o.amount), statusLabel(o.status)],
        o.status === 'overdue' ? NEGATIVE : GRAPHITE,
      )
    }
  }
...
export async function buildPendingPaymentsPdf(report: PendingReport): Promise<Buffer> {
  ...
  for (const c of report.companies) drawCompany(d, c)
