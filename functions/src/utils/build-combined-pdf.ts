...
function isNativeImage(mime: string): boolean {
  return mime.includes('jpeg') || mime.includes('jpg') || mime.includes('png')
}

async function addImagePage(out: PDFDocument, part: PdfPart): Promise<void> {
...
async function appendPdfPages(out: PDFDocument, bytes: Buffer): Promise<void> {
