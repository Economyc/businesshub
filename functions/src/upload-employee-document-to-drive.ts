...
export const uploadEmployeeDocumentToDrive = onCall(
  { region: 'us-central1', memory: '512MiB', timeoutSeconds: 60, secrets: SECRETS },
  async (request) => {
    if (!request.auth) throw new HttpsError('unauthenticated', 'Login requerido')
    const data = request.data as UploadEmployeeDocInput
    const actorUid = request.auth.uid

    if (!data?.companyId) throw new HttpsError('invalid-argument', 'companyId requerido')
    if (!data.employeeName?.trim()) throw new HttpsError('invalid-argument', 'employeeName requerido')
    if (!data.docTypeLabel?.trim()) throw new HttpsError('invalid-argument', 'docTypeLabel requerido')
